"""MCP tool adapter — wraps an MCP server tool as an Helix ``Tool``.

This adapter bridges the MCP (Model Context Protocol) tool interface with
Helix's internal ``Tool`` ABC, so MCP-provided tools are indistinguishable
...
class MCPToolAdapter(Tool):
    """Adapt an MCP tool into an Helix ``Tool``.
