"""Abstract base class for all Axon tools.

To add a new tool:

1. Create a new module under ``axon/tools/``.
2. Subclass ``Tool`` and implement ``name``, ``description``,
   ``parameters_schema``, and ``run()``.
3. Register it in ``axon/tools/registry.py`` (or dynamically at startup).
"""
...
class Tool(ABC):
    """Contract that every Axon tool must satisfy.

    Subclasses are automatically usable in the agent loop once registered
    with :class:`axon.tools.registry.ToolRegistry`.
    """
